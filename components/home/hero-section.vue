<template>
  ...
  <div v-if="loading"
    ...
    <div class="flex flex-row h-full animate-pulse">
      <div class="flex flex-col justify-center lg:w-1/2 md:w-3/4 w-full">
        <div class="h-8 bg-sky-400 rounded mb-6"></div>
        <div class="h-10 bg-sky-400 rounded mb-12"></div>
        <div class="h-8 bg-sky-400 rounded mb-12"></div>
        <div class="mb-4 flex sm:flex-none flex-col sm:flex-row">
          <div class="h-10 w-32 bg-sky-400 rounded mr-4 mb-5 sm:mb-0"></div>
          <div class="h-10 w-32 bg-sky-400 rounded sm:ml-7 ml-0"></div>
        </div>
      </div>
      <div class="md:flex hidden -mr-10 justify-end items-end md:w-1/2 w-1/4">
        <div class="h-96 lg:w-[400px] max-[1105px]:w-[300px] md:w-[250px] bg-sky-400 rounded-lg"></div>
      </div>
    </div>
  </div>
  ...
  <div v-else
    ...
    <div class="flex flex-row h-full">
      ...
      <div class="md:flex hidden -mr-10 justify-end items-end md:w-1/2 w-1/4">
        <NuxtImg :src="getImageUrl(heroData[0]?.hero_img)" format="webp" alt="profile-photo"
